  ...
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO tasks (
        ...
        status, owner, phase, priority, is_milestone, description, user_id
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
...
app.put('/api/tasks/:id', authenticateToken, checkEditPermission, (req, res) => {
  try {
    ...
    const stmt = db.prepare(`
      UPDATE tasks
      SET text = ?, type = ?, parent = ?, start_date = ?, end_date = ?,
          duration = ?, progress = ?, status = ?, owner = ?, phase = ?,
          priority = ?, is_milestone = ?, description = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `);
    ...
    db.prepare(`
      INSERT INTO activity_logs (user_id, action, task_id, details)
      VALUES (?, ?, ?, ?)
    `).run(req.user.id, 'update', id, `更新任务: ${updates.text}`);
